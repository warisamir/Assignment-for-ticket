...
exports.createTicket = async (req, res) => {
  ...
  try {
    ...
    if (seat_no > 7) {
      return res
        .status(404)
        .json({ message: "No should not be greater than 7" });
    }
    ...
    if (newTicket.length < seat_no) {
      return res
        .status(404)
        .json({
          message: "No of seats are not avilable please use less Number",
        });
    }
    let row = 1;
    let flag = false;
    while (row <= 12) {
      ...
      if (newRowTicket.length >= seat_no) {
        ...
        if (cuncurrentArray) {
          ...
          return res
            .status(200)
            .json({ message: "successfully booked ticket", data: seatsNumber });
        }
      }
      row++;
    }
    ...
    if (!flag) {
      ...
      return res
        .status(200)
        .json({ message: "successfully booked ticket", data: slidingWindow });
    }
  } catch (err) {
...
exports.deleteTicket = async (req, res) => {
  try {
    ...
    if (newTicket.length > 0) {
      await Ticket.updateMany({}, { $set: { status: false } }).sort({
        row: 1,
        seat_no: 1,
      });
